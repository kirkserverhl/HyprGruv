...
#   HYPRGRUV_LID_SUSPEND=0|1          laptop lid watch (5m grace → lock+hibernate; default 1)
...
prompt_laptop_extras() {
    ...
    if [[ -z "$WANT_LID" ]]; then
        if gum_confirm_prompt "After closing the lid: 5 minutes unlocked, then lock + hibernate? (desk dock with an external display stays on)"; then
            WANT_LID=1
        else
            WANT_LID=0
        fi
...
apply_profile_values() {
    ...
    *)
    ...
    if [[ "$machine" == "laptop" ]]; then
        ...
        blur_size=6
        shadow_enabled=true
        # Idle: dim → lock → DPMS → hibernate (see write_hypridle_conf)
        # 4m dim, 5m lock, 6m monitor off, 15m hibernate
        dim_timeout=240
        lock_timeout=300
        dpms_timeout=360
        suspend_timeout=900
        sleep_action=hibernate
        lid_grace=300
        want_deploy="${WANT_DEPLOY:-1}"
        want_lid="${WANT_LID:-1}"
    else
    ...
    write_setting hypridle_dim_timeout "$dim_timeout"
    write_setting hypridle_sleep_action "$sleep_action"
    write_setting lid_grace_sec "$lid_grace"
...
HYPRIDLE_DIM=$dim_timeout
HYPRIDLE_SLEEP_ACTION=$sleep_action
LID_GRACE=$lid_grace
...
EOF
    ...
    write_hypridle_conf "$machine" "$lock_timeout" "$dpms_timeout" "$suspend_timeout" "$sleep_action" "$dim_timeout"
    write_blur_conf "$blur_size" "$blur_passes"
    apply_deploy_marker "$want_deploy"
    ensure_user_unit "hyprgruv-idle.service"
    apply_lid_policy "$want_lid"
    systemctl --user daemon-reload 2>/dev/null || true
    systemctl --user enable hyprgruv-idle.service 2>/dev/null || true
...
write_hypridle_conf() {
    ...
    local sleep_sh="${XDG_CONFIG_HOME:-$HOME/.config}/hyprgruv/scripts/hyprgruv-sleep.sh"

    cat >"$HYPRIDLE_OUT" <<EOF
# Generated by apply-machine-profile.sh — machine-local (not stowed)
# machine=${machine}  dim=${dim_t}s  lock=${lock_t}s  dpms=${dpms_t}s  ${sleep_action}=${susp_t}s
general {
    lock_cmd = ${lock_cmd}
...
EOF
    ...
    if [[ "${dim_t}" -gt 0 ]] 2>/dev/null && command -v brightnessctl >/dev/null 2>&1; then
        cat >>"$HYPRIDLE_OUT" <<EOF
# Dim before lock
listener {
    timeout = ${dim_t}
...
apply_lid_policy() {
    ...
    local lid_unit="hyprgruv-lid-watch.service"

    # logind must ignore the lid so we can offer a 5-minute unlocked grace.
    # Immediate HandleLidSwitch=suspend also no-ops when logind reports Docked=true.
    if [[ "$want" == "1" ]]; then
        if ! command -v sudo >/dev/null 2>&1; then
            ...
            log_status "Manual: sudo cp $HYPR_DIR/lib/snapshot/99-hyprgruv-lid.conf $LOGIND_DROPIN && sudo systemctl kill -s HUP systemd-logind"
        else
            log_status "Installing logind lid=ignore drop-in (grace handled by $lid_unit)…"
            local lid_src="$HYPR_DIR/lib/snapshot/99-hyprgruv-lid.conf"
            if sudo mkdir -p /etc/systemd/logind.conf.d \
                && sudo cp "$lid_src" "$LOGIND_DROPIN" \
                && sudo chmod 644 "$LOGIND_DROPIN"
            then
        ...
        ensure_user_unit "$lid_unit"
        if [[ -e "$unit_dir/$lid_unit" ]]; then
            systemctl --user daemon-reload 2>/dev/null || true
            systemctl --user enable --now "$lid_unit" 2>/dev/null \
                && log_status "Enabled $lid_unit" \
                || log_warning "Could not enable $lid_unit"
        fi
    else
        systemctl --user disable --now "$lid_unit" 2>/dev/null || true
...
ensure_user_unit() {
    local name="$1"
    local src="$HYPR_DIR/home/.config/systemd/user/$name"
    local dst="${XDG_CONFIG_HOME:-$HOME/.config}/systemd/user/$name"
    mkdir -p "$(dirname "$dst")"
    if [[ -f "$src" ]]; then
        ln -sfn "$src" "$dst"
    fi
...
restart_hypridle() {
    ...
    if [[ -f "$unit_dir/$idle_unit" || -L "$unit_dir/$idle_unit" ]]; then
        systemctl --user daemon-reload 2>/dev/null || true
        systemctl --user enable "$idle_unit" 2>/dev/null || true
        if systemctl --user restart "$idle_unit" 2>/dev/null; then
            log_status "Restarted $idle_unit"
            return 0
        fi
    fi
    if [[ -x "$launch" ]]; then
    ...
    log_warning "hypridle launcher missing — idle config written"
}
