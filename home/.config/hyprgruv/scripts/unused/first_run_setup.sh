...
resolve_terminal() {
  local term="kitty"
  if [[ -x "$HOME/.config/hyprgruv/scripts/terminal.sh" ]]; then
    term="$("$HOME/.config/hyprgruv/scripts/terminal.sh" --print 2>/dev/null || echo kitty)"
...
SELF="$HOME/.config/hyprgruv/scripts/first_run_setup.sh"
