...
#   +----------+----------+
#   | nvim     | cmatrix  |  top-right: matrix rain → shell on keypress
#   | (left)   +----------+
#   |          | yazi     |  bottom-right
#   +----------+----------+
set -euo pipefail
...
# Pane 1: left (nvim). Pane 2: top-right (cmatrix). Pane 3: bottom-right (yazi).
# (oh-my-tmux uses base-index 1 for panes/windows.)
tmux split-window -h -t "${SESSION_NAME}:${WINDOW_NAME}" -p 45 -c "$START_DIR"
tmux split-window -v -t "${SESSION_NAME}:${WINDOW_NAME}.2" -p 38 -c "$START_DIR"

tmux send-keys -t "${SESSION_NAME}:${WINDOW_NAME}.1" "$NVIM_CMD" Enter
tmux send-keys -t "${SESSION_NAME}:${WINDOW_NAME}.2" "bash $(printf '%q' "$CMATRIX_PANE")" Enter
