...
-- Load system/matugen palette (~~/script-opts/modernz-matugen.conf) over modernz.conf.
-- Generated by matugen templates + generate-preset-colors; machine-local / gitignored.
local function apply_matugen_colors()
    local path = mp.command_native({"expand-path", "~~/script-opts/modernz-matugen.conf"})
    ...
    for line in f:lines() do
        ...
        if line ~= "" and not line:match("^#") then
            ...
            if k and v and user_opts[k] ~= nil then
                -- Strip trailing comments, but not hex colors like #RRGGBB
                if not v:match("^#") then
                    v = v:match("^(.-)%s+#") or v
                end
                v = v:match("^%s*(.-)%s*$") or v
    ...
    f:close()
    if applied > 0 then
        msg.info("modernz: applied " .. applied .. " matugen color opts")
    end
    return applied > 0
end
