...
complete -c systemctl -l show-types -d 'When showing sockets, show the type of the socket. sp Added in version 202'
...
complete -c systemctl -s i -d 'Shortcut for --check-inhibitors=no. sp Added in version 198'
...
complete -c systemctl -l wait -d 'When used with start or restart, synchronously wait for started units to term…'
...
complete -c systemctl -l no-reload -d 'When used with enable, disable, preset, mask, or unmask, do not implicitly re…'
...
complete -c systemctl -l now -d 'When used with enable, disable, mask, or reenable, also start/stop/try-restar…'
...
complete -c systemctl -l read-only -d 'When used with bind, creates a read-only bind mount. sp Added in version 248'
complete -c systemctl -l drop-in -d 'When used with edit, use NAME as the drop-in file name instead of override'
complete -c systemctl -l when -d 'When used with halt, poweroff, reboot or kexec, schedule the action to be per…'
complete -c systemctl -l stdin -d 'When used with edit, the contents of the file will be read from standard inpu…'
...
complete -c systemctl -s C -l capsule -d 'Execute operation on a capsule.  Specify a capsule name to connect to'
complete -c systemctl -l no-ask-password -d 'Do not query the user for authentication for privileged operations'
...
complete -c systemctl -l log-level -d 'described in systemd(1))'
complete -c systemctl -l log-target -d 'systemd(1))'
