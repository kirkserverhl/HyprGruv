...
complete -c ls -s f -d 'same as -a -U'
...
complete -c ls -l group-directories-first -d 'group directories before files'
...
complete -c ls -l sort -d 'change default \'name\' sort to WORD: none (-U), size (-S), time (-t), version …'
...
complete -c ls -s U -d 'do not sort directory entries'
