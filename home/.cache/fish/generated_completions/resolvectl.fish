...
complete -c resolvectl -l relax-single-label -d 'Takes a boolean parameter; used in conjunction with query'
complete -c resolvectl -l no-ask-password -d 'Do not query the user for authentication for privileged operations'
complete -c resolvectl -l json -d 'Shows output formatted as JSON'
complete -c resolvectl -s j -d 'Equivalent to --json=pretty if running on a terminal, and --json=short otherw…'
